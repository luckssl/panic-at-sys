...
class Fase1(Fase):
    def __init__(self, janela, teclado, mouse, proxima_fase=Fase2):
        super().__init__(janela, teclado, mouse, proxima_fase)
        self.background = GameImage("background_fase1.jpg")
        self.layer2 = GameImage("layer2_1.png")
        self.layer2_copia = GameImage("layer2_1.png")
        ...
        self.boss = Boss1(self.janela, "boss1.png")  # Boss (caso haja)
        self.acabou_jogo = False
        self.boss_spawn = False
        self.soundtrack = Sound("soundtrack1.ogg")
    ...
    def atualizar(self):
        ...
        if self.layer2.x < -self.layer2.width:
            self.layer2.x = self.janela.width

        if self.layer2_copia.x < -self.layer2.width:
            self.layer2_copia.x = self.janela.width
    ...
    def verificar_colisoes(self):
        if not self.player.invencible:
            ...
            if self.boss_spawn:
                ...
                if self.boss.laser_desenha:
                    if self.player.collided(self.boss.laser):
                        self.player.atingido(2)
                        self.som_colisao.play()
